...
int32_t main(int32_t argc, char **argv)
{
    ...
    {
        /** 
         * create od4 session object for communicating with other micro-services. 
         * its important that all the "cid" numbers are the same
        */
        cluon::OD4Session od4{static_cast<uint16_t>(std::stoi(commandlineArguments["cid"]))};

        /**
         * while od4 session is running
        */
        if (0 == od4.isRunning())
        {
        ...
        while (od4.isRunning())
        {
            ...
            pedalReq.position(0.24);
            od4.send(pedalReq);
            steerReq.groundSteering(-0.66); //go right
            od4.send(steerReq);
            std::this_thread::sleep_for(std::chrono::milliseconds(delay));
            if (VERBOSE)
                std::cout << " and stop." << std::endl;
